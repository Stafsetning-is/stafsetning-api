...
export class AvatarBuilder implements IAvatarBuilder {
	...
	private async createBuffer(id: string): Promise<Buffer> {
		...
		const canvas = await this.imageBuilder.buildImage(
			this,
			random,
			this.width,
			this.height
...
export class ComposeImageBuilder implements IImageBuilder {
	constructor(private imageBuilders: IImageBuilder[]) {}

	async buildImage(
		avatarBuilder: IAvatarBuilder,
		random: IRandom,
		width: number,
		height: number
	): Promise<Canvas> {
		...
		for (const imageBuilder of this.imageBuilders) {
			const image = await imageBuilder.buildImage(
				avatarBuilder,
				random,
				width,
				height
...
export class RandomImageBuilder implements IImageBuilder {
	...
	async buildImage(
		...
		const image = await this.imageBuilders[index].buildImage(
			avatarBuilder,
			random,
			width,
			height
...
export class MarginImageBuilder implements IImageBuilder {
	...
	async buildImage(
		...
		const image = await this.imageBuilder.buildImage(
			avatarBuilder,
			random,
			imgWidth,
			imgHeight
...
export class FillStyleImageBuilder implements IImageBuilder {
	constructor(private fillStyle: any) {}
...
export class RandomFillStyleImageBuilder implements IImageBuilder {
	constructor(private fillStyles: any[] = DEFAULT_COLORS) {}
...
export class CircleMaskImageBuilder implements IImageBuilder {
	constructor(private imageBuilder: IImageBuilder) {}
...
export class RoundedRectMaskImageBuilder implements IImageBuilder {
	...
	constructor(private imageBuilder: IImageBuilder, radius: number = 10) {
	...
	private static isInShade(imageData: any, x: number, y: number): boolean {
		...
		while (true) {
			...
			if (tx < 0 || ty < 0) {
				...
				const alpha =
					imageData.data[ty * imageData.width * 4 + tx * 4 + 3];
				if (alpha > 0) {
					return true;
				}
			}
...
export class GroupImageBuilder implements IImageBuilder {
	...
	constructor(pathLocation: string, private groups: string[][]) {
	...
	private static buildPartMap(partsLocation) {
		...
		return dirs.reduce((parts, ps) => {
			...
			parts[ps] = fs
				.readdirSync(dir)
				.filter((fileName) => regex.exec(fileName))
	...
	async buildImage(
		...
		for (let y = 0; y < this.gridy; y++) {
			for (let x = 0; x < this.gridx; x++) {
				const image = await this.imageBuilders.buildImage(
					avatarBuilder,
					random,
					sizex,
					sizey

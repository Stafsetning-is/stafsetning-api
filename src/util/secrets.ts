...
if (!FB_SERVICEACCOUNT_KEY) {
	logger.error(
		"No Firebase service account key specified. Set FB_SERVICEACCOUNT_KEY environment variable."
	);
	process.exit(1);
}
...
if (!FB_API_KEY) {
	logger.error(
		"No Firebase api key specified. Set FB_API_KEY environment variable."
	);
	process.exit(1);
}
